...
void main(void) 
{
    ...
    drawPaddle(paddleL,0);   // draw the paddle possition to gameArray[row][col]
    writeMatrix(0,7,0,0);   // display the paddle on the LED matrix at col 0
    drawPaddle(paddleR,15);   // draw the paddle possition to gameArray[row][col]
      ...
      /*  if(RPB2 == LO)
        {
            ...
            {
               // printf("%d\n", count);
                //printf("  boundary[%d] %d   gameArray[%d][0] %d   count %d\n", row, boundary[row], row, gameArray[row][0], count);
                //printf("gameArray[%d][0] %d    gameArray[%d][15] %d \n", row, gameArray[row][0], row, gameArray[row][15]);
            }
            printf("\n");
        }*/
       ...
       // GIE = LO;
        moveBall(START);
        writeMatrix(0,7,1,14);  // display ball on LED matrix
        //__delay_ms(50);
       // GIE = HI;
